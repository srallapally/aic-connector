...
├── (OrganizationHandler.java)     # NOT YET CREATED — planned, extends AbstractHandler
...
- docs/idm_swagger.json — PingOne IDM REST API Swagger spec. Consult for endpoint
  paths, request/response shapes, query parameters, and field names before
  implementing any HTTP call against the IDM API.
...
Unit tests live under `src/test/java` and use JUnit 5 + Mockito.

Existing tests:
- `UserSchemaHandlerTest` — validates dynamic schema parsing from docs/user.json
  (writable relationships, virtual effective* attributes, read-only fields)
- `CrestFilterTranslatorTest` — validates filter translation
  (eq, sw, co, and/or/not composition, unsupported filter rejection)
...
- pom.xml Embed-Directory=lib fix
- Unit tests for UserSchemaHandler and CrestFilterTranslator

### Not yet created
- OrganizationHandler — file does not exist yet
- OrganizationSchemaHandler — file does not exist yet

### Next
1. Deploy to RCS, verify availableConnectors
2. Test user CRUD against live AIC tenant
3. Test role CRUD against live AIC tenant
4. Create OrganizationSchemaHandler (nameAttribute=name)
5. Create OrganizationHandler (nameAttribute=name)
6. Wire organization in PingAICConnector (uncomment imports + handler init + dispatch)
