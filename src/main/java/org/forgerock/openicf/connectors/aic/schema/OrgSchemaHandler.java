// src/main/java/org/forgerock/openicf/connectors/aic/schema/OrgSchemaHandler.java
...
public class OrgSchemaHandler {

    private final CrestHttpClient http;
    private final String realm;
    private Map<String, String> relationshipCollections = Collections.emptyMap();
    private Map<String, String> singularRelationships = Collections.emptyMap();
    private Set<String> readOnlyRelationships = Collections.emptySet();

    public OrgSchemaHandler(CrestHttpClient http, String realm) {
        this.http = http;
        this.realm = realm;
    }

    public ObjectClassInfo getObjectClassInfo() {
        JsonNode schema = http.get("/openidm/schema/managed/" + realm + "_organization", null);
        ...
        ocib.setType("organization");

        Map<String, String> relMap = new LinkedHashMap<>();
        Map<String, String> singRelMap = new LinkedHashMap<>();
        ...
        for (Iterator<Map.Entry<String, JsonNode>> it = properties.fields(); it.hasNext(); ) {
            ...
            if ("relationship".equals(type)) {
                JsonNode rcArray = propDef.path("resourceCollection");
                if (!rcArray.isArray() || rcArray.isEmpty()) {
                    continue;
                }
                String collectionPath = rcArray.path(0).path("path").asText(null);
                ...
                singRelMap.put(propName, collectionPath);
                relMap.put(propName, collectionPath);

                AttributeInfoBuilder rab = new AttributeInfoBuilder();
                rab.setName(propName);
                rab.setType(String.class);
                rab.setMultiValued(false);
                rab.setReturnedByDefault(false);
                rab.setCreateable(true);
                rab.setUpdateable(true);
                ocib.addAttributeInfo(rab.build());
        ...
        this.singularRelationships = Collections.unmodifiableMap(singRelMap);
    ...
    public Map<String, String> getSingularRelationships() {
        return singularRelationships;
    }
